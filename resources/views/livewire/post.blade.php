        ...
        <div class="overflow-x-auto">
          <table class="table-auto items-center bg-transparent w-full border-collapse ">
            ...
            <tr class="border border-solid text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
              ...
              <td class="px-6 py-3">Slug</td>
              <td class="px-6 py-3">Created at</td>
              <td class="px-6 py-3">Updated at</td>
            ...
            @foreach ($records as $record)
              <tr>
                ...
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">{{ $record->title }}</td>
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">{{ $record->slug }}</td>
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{{ $record->created_at }}</td>
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{{ $record->updated_at }}</td>
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  <a href="{{ route('admin.post.edit', $record->id) }}"
                     class="inline-flex items-center py-2 pl-2 pr-1 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition">
                    <i class="fas fa-edit"></i>
                  </a>
                  <button wire:click="delete({{ $record->id }})"
                          class="inline-flex items-center justify-center p-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 focus:outline-none focus:border-red-700 focus:ring focus:ring-red-200 active:bg-red-600 disabled:opacity-25 transition">
